"""
Orbit Wars - agent_v46

Candidate: Corrected ROI denominator for neutral planets
Base: agent_v42

Bug found: the _roi denominator uses `t.ships + t.production * travel + 1` for ALL
targets. This correctly estimates the cost of capturing ENEMY planets (which produce
ships during fleet travel). But neutral planets have static garrison — they only
produce ships once owned. The game engine confirms: production is added only for
planets with owner != -1 (line 514 of orbit_wars.py).

Fix: use `t.ships + 1` as the cost for neutrals (owner == -1), and keep
`t.ships + t.production * travel + 1` for enemy-owned planets.

Effect: neutrals — especially high-production ones at medium distance — get correctly
high ROI scores. Previously they were heavily discounted by the spurious `prod*travel`
cost even though that cost never materializes. The relative ordering of targets shifts
to more accurately reflect actual capture cost.
"""
...
def _roi(t, bx, by, mine):
    ...
    if t.owner == -1:
        # Neutral: garrison is static (neutrals don't produce); actual cost = ships + 1
        cost = max(1.0, t.ships + 1)
    else:
        # Enemy: garrison accumulates during travel
        cost = max(1.0, t.ships + t.production * travel + 1)
    return (t.production ** 2) * max(1.0, 100.0 - travel) / cost
