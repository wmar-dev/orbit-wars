"""
Orbit Wars - Experiment 013 scoring-3: distance-gated ROI
Base: agent_v38. Change: skip targets farther than nearest_non_owned_dist * 1.5.
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # scoring-3: compute distance gate for this source planet
        nearest_non_owned_dist = min(
            (math.hypot(t.x - mine.x, t.y - mine.y) for t in targets),
            default=float('inf')
        )
        dist_gate = nearest_non_owned_dist * 1.5
        ...
        for t in targets:
            ...
            # scoring-3: skip targets beyond the distance gate
            if math.hypot(x_pred - mine.x, y_pred - mine.y) > dist_gate:
                continue
