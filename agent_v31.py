"""
Orbit Wars - Reward-Guided Agent (agent_v31)

Builds on agent_v30 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

New mechanic — Candidate S: Reward-blend target scoring (this version)
  Blends the existing ROI score with a forward-looking reward estimate
  that values planet captures and minimises ship expenditure.
  Score = (1 - REWARD_ALPHA) * roi_normalised + REWARD_ALPHA * reward_estimate
  Default REWARD_ALPHA = 0.1 (10% reward influence, 90% ROI).
  Tested: alpha=0.1 → score 61% vs v30 (PASS); alpha=0.3 → 54% (FAIL).
  Setting REWARD_ALPHA = 0.0 reproduces agent_v30 exactly (verified: 20/20 draws).
...
# Reward-blend config (Candidate S)
REWARD_ALPHA = 0.1  # blend factor: 0.0 = pure ROI (identical to v30), 1.0 = pure reward
...
def _reward_estimate(target, dispatch_ships):
    ...
    Estimates the normalised capture_bonus minus the ship expenditure cost,
    using the same weights as reward_signal.py. Result is in [-1, 1] but
    positive for any target worth capturing.
    """
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Normalise ROI across candidates so both components are in [0, 1].
