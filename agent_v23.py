"""
Orbit Wars - Candidate K: Enemy-Territory Priority When Winning (agent_v23)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Enemy-territory priority (Candidate K): When own_total >= 1.5 * enemy_total
    (winning decisively), apply a 1.5x multiplier to the ROI score of
    enemy-owned targets. Neutral planet scoring is unchanged. This biases
    target selection toward enemy production centers when ahead, ending games
    faster than neutral expansion would.

Hypothesis: The current ROI formula treats neutrals and enemy planets
identically. When significantly winning, capturing enemy planets directly
shrinks their production and forces an endgame; this should convert winning
positions into faster wins.
"""
...
def _roi_k(t, bx, by, mine, player, winning):
    """Candidate K: 1.5x multiplier for enemy-owned targets when winning."""
    base = _roi(t, bx, by, mine)
    if winning and t.owner != -1 and t.owner != player:
        return base * 1.5
    return base
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Candidate K: use ROI with enemy-priority multiplier.
        best_target, bx, by = max(
            candidates,
            key=lambda item: _roi_k(item[0], item[1], item[2], mine, player, winning),
        )
