"""
Orbit Wars - Combined Agent (agent_v20)

Builds on agent_v15 (single-sender coordination + all agent_v10 safety guards).

Stacked mechanics (only mechanics that passed ≥55% vs agent_v15 are included):
  - Speed-corrected orbit lead (Candidate E, agent_v16): Computes
    fleet_speed(target.ships + 1) per target inside the candidates loop, fixing
    a bug where travel-time was computed from the source planet's full ship count
    rather than the actual launched fleet size. Ensures fleets aim at the correct
    predicted position for orbiting targets.
  - Capture-ROI scoring (Candidate H, agent_v19): Replaces production/distance
    scoring with production * max(1, 100 - travel_turns) / max(1, target.ships +
    target.production * travel_turns + 1). Rewards fast, cheap captures with more
    remaining production time.

Candidates NOT included (failed ≥55% threshold):
  - Candidate F (transit-adjusted fleet sizing): 15% win rate vs v15 — FAIL
  - Candidate G (adaptive range expansion): 0% win rate vs v15 — FAIL
...
EPSILON = 1e-6
RANGE_FACTOR = 2.0
GARRISON_FLOOR_FACTOR = 5  # production multiplier for single-sender garrison floor
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Fallback: if no assigned target is in range, try any assigned target ignoring range.
        if not candidates:
            for t in targets:
                if best_sender.get(t.id) != mine.id:
                    continue
