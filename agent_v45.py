"""
Orbit Wars - agent_v45

Candidate: Urgency-weighted ROI for neutral planets
Base: agent_v42

Change: for each neutral planet target, compute an "urgency boost" based on our
distance advantage over the nearest enemy planet. Specifically:
  urgency = max(0, 1 - our_time / enemy_min_time) * URGENCY_FACTOR
  adjusted_roi = base_roi * (1 + urgency)

If we can reach the neutral much faster than any enemy planet, urgency is high → big ROI
boost → we prioritize capturing these "uncontested" planets first.
If the enemy is closer, urgency = 0 → no boost → base ROI used.

Only applied to neutral (owner == -1) planets. Enemy-owned planets are already scored
correctly by the base ROI (we want to attack weakened ones regardless of enemy proximity).

URGENCY_FACTOR = 0.4  (max 40% ROI boost when we have a large speed advantage)
"""
...
REWARD_ALPHA = 0.1
ANGLE_EPSILON = 0.1
URGENCY_FACTOR = 0.4  # maximum fractional ROI boost for uncontested neutrals
...
def agent(obs):
    ...
    def _urgency_boost(t, mine_x, mine_y):
        """Boost ROI for neutrals we can reach faster than the nearest enemy planet."""
        if t.owner != -1 or not enemy_planets:
            return 0.0
        speed = fleet_speed(t.ships + 1)
        our_time = math.hypot(t.x - mine_x, t.y - mine_y) / speed
        enemy_min_time = min(
            math.hypot(ep.x - t.x, ep.y - t.y) / fleet_speed(t.ships + 1)
            for ep in enemy_planets
        )
        if enemy_min_time < EPSILON:
            return 0.0
        ratio = our_time / enemy_min_time
        # ratio < 1: we're closer → boost (lock in the capture before enemy can contest)
        return max(0.0, 1.0 - ratio) * URGENCY_FACTOR
    ...
    for mine in my_planets:
        ...
        # Urgency-boosted ROI for neutral planets
        roi_scores = []
        for t, bx, by in candidates:
            base = _roi(t, bx, by, mine)
            boost = _urgency_boost(t, mine.x, mine.y)
            roi_scores.append((base * (1.0 + boost), t, bx, by))
