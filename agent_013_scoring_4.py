"""
Orbit Wars - Experiment 013 scoring-4: linear normalised blend
Base: agent_v38. Change: score = 0.67*prod_norm + 0.33*(1-dist_norm), removing quadratic prod.
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # scoring-4: linear normalised blend (0.67 * prod_norm + 0.33 * (1 - dist_norm))
        _map_max_prod = max(p.production for p in planets) or 1.0
        _map_max_dist = 141.5  # board diagonal (100x100), fixed normalisation constant

        def _linear_score(t, bx, by, mine, _mmp=_map_max_prod, _mmd=_map_max_dist):
            prod_norm = t.production / _mmp
            dist_norm = min(1.0, math.hypot(bx - mine.x, by - mine.y) / _mmd)
            return 0.67 * prod_norm + 0.33 * (1.0 - dist_norm)

        scored = [(_linear_score(t, bx, by, mine), t, bx, by) for t, bx, by in candidates]
        best_roi, best_target, bx, by = max(scored, key=lambda x: x[0])
