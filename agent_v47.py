"""
Orbit Wars - agent_v47

Candidate: Production-adjusted fleet sizing for enemy planets
Base: agent_v42

Bug found: when attacking enemy-owned planets, we send `target.ships + 1` ships.
But enemy planets produce ships every turn while our fleet is in transit. By the
time the fleet arrives, the enemy has `target.ships + production * travel_turns`
garrison. Our undersized fleet deals damage but fails to capture.

Fix: for non-neutral targets (owner != -1), compute the garrison at estimated
arrival time and size the fleet accordingly:
  ships_needed = int(target.ships + target.production * travel) + 1

Since a larger fleet is faster, we also recompute the orbit-lead with the corrected
fleet speed to avoid aiming at the wrong position (orbiting targets move significantly
during the faster travel time).

For neutral targets (owner == -1), the garrison is static; keep ships_needed = ships + 1.
"""
