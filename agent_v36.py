"""
Orbit Wars - Candidate U Round 6: Threat-Aware Garrison Floor (agent_v36)

Builds on agent_v33.

New mechanic — Candidate U (Round 6): Threat-aware garrison floor
  ...
  to each owned planet's position to infer the threat without a to_planet_id field.
  Raises the garrison floor for threatened planets:
    floor = max(GARRISON_FLOOR_FACTOR * production, incoming_enemy_ships)
  This prevents dispatch from planets about to be attacked (agent_v33 may drain a
  planet's garrison offensively right before an enemy fleet arrives).
  Does NOT add defensive dispatch moves (unlike Candidate I: reactive defense, which
  failed at 16% vs v20 and 5% vs v32 by diverting ships away from offense entirely).
...
REWARD_ALPHA = 0.1
ANGLE_EPSILON = 0.1  # radians: threshold for fleet-to-planet angle matching
...
def agent(obs):
    ...
    # Candidate U (Round 6): build threat dict from obs.fleets (enemy fleets → owned planets)
    threat = {}
    for f in raw_fleets:
