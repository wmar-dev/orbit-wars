"""
Orbit Wars - Fleet Safety Overhaul (agent_v10)

Builds on agent_v9 (full-ray sun check + OOB rejection, 94% vs main.py).

Fixes vs agent_v9:
  1. Intermediate planet obstruction: _path_safe() now checks every non-target
     planet along the full ray, not just the sun. A fleet aimed at a target
     that passes within planet.radius + PLANET_MARGIN of any other planet is
     rejected. This eliminates a class of transit losses that v9 never detected.
  2. Orbit-lead travel_turns refinement: one iteration of correction — predict
     the target at t0 (estimated travel time using current distance), recompute
     distance to the predicted position, derive t1, then use the position
     predicted at t1 as the final aim point. Reduces intercept misses caused by
     using the planet's current position as the distance baseline.
  3. Comet path index clamping: future_idx is clamped to len(path)-1 and an
     empty-path guard is added to prevent index errors near comet departure.

Sun check (from v9, confirmed correct):
  The full ray from source to board edge is checked against SUN_EXCLUSION=12.0,
  ensuring the fleet won't hit the sun even after passing the predicted target.

OOB guard (from v9, confirmed correct):
  Inclusive [0, 100] bounds are correct per CONTEST.md (board is 100x100,
  coordinates at exactly 0 or 100 are on the boundary, not out of bounds).
"""
...
def _segment_dist_to_sun(ax, ay, bx, by):
    """Minimum distance from segment A->B to the sun center."""
    return _segment_dist_to_point(ax, ay, bx, by, _SUN_X, _SUN_Y)


def _ray_exits_board(ox, oy, angle):
    """Return the (x, y) where a ray from (ox, oy) at angle exits the 100x100 board."""
...
def _path_safe(ox, oy, tx, ty, all_planets=None, target_id=None, source_id=None):
    ...
    - The segment from origin to target clears every intermediate planet by
      planet.radius + PLANET_MARGIN. Only planets between source and target
      can intercept the fleet before it reaches the intended destination.
      Source and target planets are excluded from this check.
    """
    if not (0 <= tx <= BOARD_SIZE and 0 <= ty <= BOARD_SIZE):
        return False

    angle = math.atan2(ty - oy, tx - ox)
    # Full ray to board edge — fleet keeps flying past the target.
    ex, ey = _ray_exits_board(ox, oy, angle)

    # Sun clearance along the full ray (fleet keeps going after passing target).
    if _segment_dist_to_sun(ox, oy, ex, ey) < SUN_EXCLUSION:
        return False

    # Intermediate planet obstruction: check segment source→target only.
    # Planets beyond the target cannot intercept the fleet before arrival.
    if all_planets:
...
def _refined_orbit_lead(t, mine, initial_planets_map, angular_velocity, speed):
    """
    Return (x_pred, y_pred) using one refinement step for orbit-lead intercept.
    t0 estimates travel time from current distance; t1 uses distance to the t0
    predicted position, giving a more accurate intercept for orbiting planets.
    """
...
def _comet_predicted_pos(comet_planet, comet_path_lookup, travel_turns):
    """
    Return (x_pred, y_pred, valid) for a comet target.
    valid=False means the comet is leaving too soon to intercept safely.
    """
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Build candidates with orbit-lead + comet path prediction.
        candidates = []
        for t in targets:
        ...
        # Fallback: any safe target ignoring range.
        if not candidates:
            for t in targets:
                dist = math.hypot(t.x - mine.x, t.y - mine.y)
                travel_turns = dist / speed
