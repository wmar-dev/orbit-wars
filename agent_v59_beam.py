"""
Orbit Wars - agent_v59_beam

Experiment C: Beam search over candidate action sets using a forward simulator.
Base: agent_v58 (affordability fallback, 58% vs v57)

Change: Instead of executing the greedy dispatch directly, generates up to
BEAM_CANDIDATES action sets (greedy baseline, per-planet target swaps, swarm
variants, hold-all), simulates BEAM_DEPTH turns forward for each using a fast
forward model (_SimState), scores each by projected production advantage, and
executes the best-scoring set. Falls back to greedy if wall time > BEAM_TIMEOUT_MS.

Timing: baseline 0.29ms/turn; 30 candidates × 5 turns × ~0.03ms = ~4ms overhead.
"""
