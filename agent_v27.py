"""
Orbit Wars - Candidate P: 3-Iteration Orbit Lead (agent_v27)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - 3-iteration orbit lead (Candidate P): Extends _refined_orbit_lead from
    2 iterations to 3. Each iteration re-estimates travel time using the
    predicted planet position from the prior step. Reduces aim error for
    fast-orbiting planets unconditionally.

Hypothesis: More iterations = more accurate fleet aim = fewer missed captures.
Unconditional improvement with no strategic tradeoff.
"""
