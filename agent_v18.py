"""
Orbit Wars - Candidate G: Adaptive Range Expansion (agent_v18)

Builds on agent_v15 (single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Adaptive range expansion (Candidate G): Computes own_ships / enemy_ships ratio
    each turn. When ratio >= 1.5 (winning decisively), expands range_factor to 3.5
    to press the advantage across the map. When ratio <= 0.7 (losing), contracts to
    1.5 to focus on nearby targets. Otherwise keeps range_factor = 2.0 (baseline).
    Enemy ships = opponent-owned planets only (not neutral).
...
def agent(obs):
    ...
    # Adaptive range factor based on own/enemy ship ratio (Candidate G).
    ...
    range_factor = 3.5 if ratio >= 1.5 else 1.5 if ratio <= 0.7 else RANGE_FACTOR
