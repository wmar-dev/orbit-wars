...
- [x] T001 Create `agent_v64.py` as a copy of `agent_v63.py` with all round 4 experiment toggles added (OPPONENT_MODEL_V3_ENABLED=False, MULTI_TURN_PLAN_ENABLED=False, PHASE_DETECTION_ENABLED=False), all existing toggles preserved, WEIGHTED_EVAL_FIXED_ENABLED=True
...
- [x] T002 Create experiment log `experiments/2026-06-06-experiments-round4.md` with round 4 template: hypothesis table for 3 experiments, baseline (v63), per-experiment sections for results (PASS/DISCARD with win rate), combined config section, and slawekbiel sweep section
- [x] T003 [P] Update `Makefile` AGENT variable to `agent_v64.py` (temporary — will revert if no experiments pass)
...
- [x] T004 [US1] Add `_sim_opponent_step_v3` in `agent_v64.py` that iterates enemy planets, computes surplus with garrison floor (gff ramp), selects targets by ROI (`_roi`-style production/distance scoring), computes fleet speed and ETA, and dispatches path-safe fleets — replaces the current nearest-target surplus-only model
- [x] T005 [US1] Wire `_sim_opponent_step_v3` into `_SimState.step()` — when `OPPONENT_MODEL_V3_ENABLED=True`, call v3 instead of v2; when False, fall back to existing v2 behavior
- [x] T006 [US1] Run independent eval vs v63: `python eval.py agent_v64.py agent_v63.py --games 50 --swap --timing` — log win rate and timing to experiment log — **DISCARDED (34%)**
...
- [x] T007 [US2] Add skip-candidate generation in `_gen_beam_candidates` in `agent_v64.py`: for each mine, produce an alternative candidate where that mine sends no fleet (zero dispatch). The skip candidate's dispatches list excludes the mine; its moves list excludes that mine's move. Surrounding mines still dispatch normally.
- [x] T008 [US2] Wire `MULTI_TURN_PLAN_ENABLED` toggle into `_gen_beam_candidates`: when True, append skip candidates to the candidate list; when False, behavior unchanged
- [x] T009 [US2] Run independent eval vs v63: `python eval.py agent_v64.py agent_v63.py --games 50 --swap --timing` — log win rate and timing to experiment log — **KEPT (54%)**
...
- [x] T010 [US3] Add `_detect_phase()` helper in `agent_v64.py` that computes `pct_owned = own_planets / max(1, non_neutral_count)` and detects three phases: Expansion (<40%), Mid-game (40-80%), Elimination (>80% or ≤1 opponent alive)
- [x] T011 [US3] Integrate phase detection into `_greedy_moves` garrison floor computation: Expansion uses normal 1.5× ramp to 400; Mid-game reduces ramp ceiling; Elimination further reduces and disables splinter dispatch — respect `PHASE_DETECTION_ENABLED` toggle
- [x] T012 [US3] Run independent eval vs v63: `python eval.py agent_v64.py agent_v63.py --games 50 --swap --timing` — log win rate and timing to experiment log — **DISCARDED (48%)**
...
- [x] T013 Enable all passing experiment toggles (MULTI_TURN_PLAN_ENABLED) in `agent_v64.py`, run combined eval vs v63: `python eval.py agent_v64.py agent_v63.py --games 50 --swap --timing` — log combined win rate — **54% vs v63**
- [x] T014 [P] Run opponent sweep for combined config: 20 games each vs slawekbiel, sigmaborov, dylanxue04, yusufmurtaza — log win rates for each in experiment log — **100% vs 3/4, 0% vs slawekbiel**
- [x] T015 If combined win rate >52%, update `Makefile` AGENT to `agent_v64.py` (permanent) and `README.md` with v64 row in agent table; otherwise revert Makefile from T003 — **v64 row added, Makefile set to v64**
- [x] T016 Update `AGENTS.md` SPECKIT block with results summary
