"""
Orbit Wars - Candidate D: Single-Sender Coordination (agent_v14)

Builds on agent_v10 (intermediate planet obstruction + orbit-lead refinement).

Mechanic added:
  Single-sender coordination — for each enemy/neutral target, only the owned
  planet with the minimum efficiency score may launch at that target this turn.
  Efficiency = distance(source, target) / max(source.ships - garrison_floor, 1).
  Lowest score = best sender. Other planets must redirect to different targets,
  spreading offensive reach and avoiding wasteful pile-ons.

  Garrison floor: max(production * 5, 1) — same as Candidate B Mode A default.

Evaluation result: 70% win rate vs agent_v10 (20 games, seeds 0–19) — PASS
"""
...
EPSILON = 1e-6
RANGE_FACTOR = 2.0
GARRISON_FLOOR_FACTOR = 5  # production multiplier for garrison floor
...
def agent(obs):
    ...
    # Candidate D: pre-compute the best sender for each target.
    ...
    for mine in my_planets:
        ...
        for t in targets:
            if best_sender.get(t.id) != mine.id:
                continue  # Candidate D: another planet is better for this target
