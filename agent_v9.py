"""
Orbit Wars - Fleet Path Safety Fix (agent_v9)

Builds on agent_v8 (orbit-lead + comet opportunism, 90% win rate).

Bug fixes vs agent_v8:
  1. Full-ray sun check: the previous segment check (source → predicted_pos) only
     tested up to the target. Fleets travel in a straight line past the target until
     hitting something or exiting the board, so the check now covers the full ray to
     the board edge.
  2. Out-of-bounds predicted positions: orbit-lead and comet path prediction can
     produce (x, y) outside [0, 100]. Those targets are now rejected before launching.

Both checks are unified in _path_safe(), which replaces all _segment_dist_to_sun()
calls in the candidate-selection logic.
"""
...
def _ray_exits_board(ox, oy, angle):
    ...
    Used to check whether the fleet path crosses the sun before leaving the board.
    """
...
def _path_safe(ox, oy, tx, ty):
    """
    Return True if the fleet path from (ox, oy) aimed at (tx, ty) is safe:
    - The ray from origin through target stays outside SUN_EXCLUSION until it
      reaches the target (or exits the board if target is beyond).
    - The predicted target position is inside the board.
    """
    if not (0 <= tx <= BOARD_SIZE and 0 <= ty <= BOARD_SIZE):
        return False
    # Check sun clearance along the full ray to board edge (fleet keeps going past target).
    ex, ey = _ray_exits_board(ox, oy, math.atan2(ty - oy, tx - ox))
    return _segment_dist_to_sun(ox, oy, ex, ey) >= SUN_EXCLUSION
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        if mine.id in evacuate_next_turn:
            sun_safe = [
                t for t in targets
                if _path_safe(mine.x, mine.y, t.x, t.y)
            ]
            if not sun_safe:
                continue
            best = max(
        ...
        for t in targets:
            ...
            if dist <= max_range and _path_safe(mine.x, mine.y, x_pred, y_pred):
        ...
        if not candidates:
            for t in targets:
                ...
                if t.id in comet_planet_ids:
                    ...
                    x_pred, y_pred = _predict_planet_pos(t, initial_planets_map, angular_velocity, travel_turns)
                if _path_safe(mine.x, mine.y, x_pred, y_pred):
                    candidates.append((t, x_pred, y_pred))
