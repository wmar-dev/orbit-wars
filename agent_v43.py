"""
Orbit Wars - agent_v43

Candidate: Multi-fleet dispatch
Base: agent_v42

Change: instead of sending one fleet per source planet (best ROI target only),
send to ALL assigned targets the planet can afford while maintaining the garrison floor.
Sorted by blended ROI score, send each fleet while (remaining_ships - ships_needed) >= floor.

Rationale: a planet with high surplus currently idles after one send. If it's the
best sender for multiple targets, it can capture two or more planets in one turn.
"""
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Multi-fleet dispatch: send to all affordable candidates (sorted by score)
        floor_val = max(mine.production * GARRISON_FLOOR_FACTOR, threat.get(mine.id, 0))
        remaining = mine.ships

        for _, best_target, bx, by in sorted(roi_scores, key=blended_key, reverse=True):
            ships_needed = best_target.ships + 1
            if remaining - ships_needed < floor_val:
                continue  # not enough surplus for this target; try smaller ones
