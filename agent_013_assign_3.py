"""
Orbit Wars - Experiment 013 assign-3: top-2 senders per target (split capture cost)
Base: agent_v38. Change: allow best + second-best sender to split ships_needed.
...
def agent(obs):
    ...
    best_sender = {}
    second_sender = {}  # assign-3: track second-best sender per target
    for t in targets:
        ...
        for src in my_planets:
            ...
            if score < best_score:
                second_score, second_pid = best_score, best_pid
                best_score, best_pid = score, src.id
            elif score < second_score:
                second_score, second_pid = score, src.id
        if best_pid is not None:
            best_sender[t.id] = best_pid
        if second_pid is not None:
            second_sender[t.id] = second_pid
    ...
    # assign-3: second-sender pass — second-best sender for each target also contributes
    MIN_CONTRIB = 10
    already_dispatched = {m[0] for m in moves}
    for t in targets:
        sec_pid = second_sender.get(t.id)
        if sec_pid is None:
            continue
        sec_planet = next((p for p in my_planets if p.id == sec_pid), None)
        if sec_planet is None or sec_planet.id in departing_this_turn:
            continue
        floor = max(sec_planet.production * GARRISON_FLOOR_FACTOR, threat.get(sec_planet.id, 0))
        if sec_planet.ships - floor < MIN_CONTRIB:
            continue
        # use orbit-lead position for the target
        speed_v = fleet_speed(MIN_CONTRIB)
        if t.id in comet_planet_ids:
            tx, ty, valid = _comet_two_pass(t, sec_planet.x, sec_planet.y, comet_path_lookup, speed_v)
            if not valid:
                continue
        else:
            tx, ty = _converged_orbit_lead(t, sec_planet, initial_planets_map, angular_velocity, speed_v)
        if not _path_safe(sec_planet.x, sec_planet.y, tx, ty, all_planets=planets, target_id=t.id, source_id=sec_planet.id):
            continue
        sec_angle = math.atan2(ty - sec_planet.y, tx - sec_planet.x)
        moves.append([sec_planet.id, sec_angle, MIN_CONTRIB])
