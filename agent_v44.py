"""
Orbit Wars - agent_v44

Candidate: Race filtering for neutral planets
Base: agent_v42

Change: before adding a neutral planet to candidates, check whether any in-flight
enemy fleet is heading toward that planet's predicted position AND will arrive before us.
If so, skip the target — the enemy will capture it first, so our fleet would be wasted.

Parameters:
  RACE_ANGLE_EPS = 0.12 rad  — tighter than threat detection (0.1) to reduce false positives
  Only applied to neutral (owner == -1) planets; enemy-owned planets can't be "won" by
  a different enemy fleet.
"""
...
REWARD_ALPHA = 0.1
ANGLE_EPSILON = 0.1
RACE_ANGLE_EPS = 0.12  # radians: threshold for "enemy fleet heading toward this target"
...
def agent(obs):
    ...
    threat = {}
    # Build enemy fleet info: (x, y, angle, speed) for race detection
    enemy_fleet_info = []
    for f in raw_fleets:
        ...
        enemy_fleet_info.append((f_x, f_y, f_angle, f_speed))
        for p in my_planets:
            expected = math.atan2(p.y - f_y, p.x - f_x)
            if _angle_diff(f_angle, expected) < ANGLE_EPSILON:
                threat[p.id] = threat.get(p.id, 0) + f_ships

    def _enemy_wins_race(t_x, t_y, our_dist, our_speed):
        our_arrival = our_dist / our_speed
        for f_x, f_y, f_angle, f_speed in enemy_fleet_info:
            expected = math.atan2(t_y - f_y, t_x - f_x)
            if _angle_diff(f_angle, expected) < RACE_ANGLE_EPS:
                enemy_dist = math.hypot(t_x - f_x, t_y - f_y)
                enemy_arrival = enemy_dist / f_speed
                if enemy_arrival < our_arrival:
                    return True
        return False
    ...
    for mine in my_planets:
        ...
        for t in targets:
            ...
            if not _path_safe(mine.x, mine.y, x_pred, y_pred, all_planets=planets, target_id=t.id, source_id=mine.id):
                continue

            # Race filter: skip neutral planets where an enemy fleet will arrive first
            if t.owner == -1 and enemy_fleet_info:
                our_dist = math.hypot(x_pred - mine.x, y_pred - mine.y)
                if _enemy_wins_race(x_pred, y_pred, our_dist, speed_for_lead):
                    continue
