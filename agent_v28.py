"""
Orbit Wars - Candidate Q: No Range Limit (agent_v28)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - No range limit (Candidate Q): Removes the nearest_dist * RANGE_FACTOR
    distance constraint. All safe targets are candidates regardless of distance.
    ROI formula already discounts distance via (100-travel_turns), so the range
    limit is redundant and only prevents attacking globally-best distant targets.

Hypothesis: Range limit prevents attacking isolated high-value planets. ROI
naturally prefers closer targets; removing the explicit cap enables globally
optimal targeting.
"""
