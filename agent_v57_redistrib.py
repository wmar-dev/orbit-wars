"""
Orbit Wars - agent_v57_redistrib

Variant B: Redistribution (ship consolidation to frontline)
Base: agent_v56 (iterative comet intercept)

Change: After offensive dispatch, any planet with surplus > REDISTRIB_THRESHOLD
and no offensive target this turn sends half its surplus to the friendly planet
with the highest frontline score: production / (min_dist_to_enemy + 1).
This consolidates ships at forward positions rather than leaving them idle on
safe backline planets.

Hypothesis: Concentrated force at the frontline enables larger attacks and
faster response to enemy incursions, improving win rate via ship efficiency.
"""
...
def _redistrib_target_score(f, enemy_planets):
    if not enemy_planets:
        return 0.0
    min_dist = min(math.hypot(f.x - e.x, f.y - e.y) for e in enemy_planets)
    return f.production / (min_dist + 1.0)
...
def agent(obs):
    ...
    comet_ids_set = set(comet_path_lookup.keys())
    for mine in my_planets:
        if mine.id in dispatched_this_turn:
            continue
        ...
        if incoming > 0:
            continue
        buffer = 0
        mine_floor = mine.production * GARRISON_FLOOR_FACTOR
        surplus = mine.ships - mine_floor
        if surplus <= REDISTRIB_THRESHOLD:
            continue
        if enemy_planets:
            my_dist_to_enemy = min(math.hypot(mine.x - e.x, mine.y - e.y) for e in enemy_planets)
            if my_dist_to_enemy < 35.0:
                continue
        transfer = max(1, surplus // 3)
        best_rt = None
        best_rt_score = float('-inf')
        best_rt_pos = (0.0, 0.0)
        for f in my_planets:
            if f.id == mine.id:
                continue
            if f.id in departing_this_turn or f.id in evacuate_this_turn or f.id in comet_ids_set:
                continue
            score = _redistrib_target_score(f, enemy_planets)
            if score <= best_rt_score:
                continue
            speed_r = fleet_speed(transfer)
            x_pred, y_pred = _converged_orbit_lead(f, mine, initial_planets_map, angular_velocity, speed_r)
            if not _path_safe(mine.x, mine.y, x_pred, y_pred, all_planets=planets, target_id=f.id, source_id=mine.id):
                continue
            best_rt_score = score
            best_rt = f
            best_rt_pos = (x_pred, y_pred)
        if best_rt is None:
            continue
        angle = math.atan2(best_rt_pos[1] - mine.y, best_rt_pos[0] - mine.x)
        moves.append([mine.id, angle, transfer])
