"""
Orbit Wars - agent_v57_cooldown

Variant D: Departure cooldown (1-turn minimum between dispatches)
Base: agent_v56 (iterative comet intercept)

Change: Module-level _last_dispatch dict tracks the last turn each planet
dispatched a fleet. If step - _last_dispatch[planet_id] < COOLDOWN_TURNS,
skip that planet for offensive dispatch. Exemptions: comet evacuation and
departing_this_turn. The original Planet Wars winner described this constraint
as "the most important one-character change", suppressing oscillation patterns.

Hypothesis: Preventing consecutive single-turn dispatches from the same planet
reduces wasted small fleets and forces the agent to accumulate sufficient force
before attacking.
"""
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        if mine.id not in evacuate_this_turn and step - _last_dispatch.get(mine.id, -999) < COOLDOWN_TURNS:
            continue
