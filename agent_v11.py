"""
Orbit Wars - Candidate A: Redundant Fleet Avoidance (agent_v11)

Builds on agent_v10 (intermediate planet obstruction + orbit-lead refinement).

Mechanic added:
  Redundant fleet avoidance — before evaluating a target, sum all friendly
  in-flight fleet ships whose destination == target. If the sum already covers
  the target (>= target.ships + 1), skip that target this turn. This prevents
  piling multiple fleets onto already-won engagements and conserves source ships
  for other targets.
...
def agent(obs):
    ...
    # Build en-route coverage: sum friendly fleet ships per destination planet id.
    en_route_coverage = {}
    for fleet in raw_fleets:
        ...
        if owner == player:
            en_route_coverage[dest_id] = en_route_coverage.get(dest_id, 0) + ships
    ...
    for mine in my_planets:
        ...
        candidates = []
        for t in targets:
            # Candidate A: skip if already covered by friendly en-route fleets.
            if en_route_coverage.get(t.id, 0) >= t.ships + 1:
                continue
        ...
        if not candidates:
            for t in targets:
                # Candidate A: skip if already covered.
                if en_route_coverage.get(t.id, 0) >= t.ships + 1:
                    continue
