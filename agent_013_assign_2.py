"""
Orbit Wars - Experiment 013 assign-2: surplus-gated secondary senders (MIN_CONTRIB=10)
Base: agent_v38. Change: planets with surplus > 10 may also send MIN_CONTRIB to top target.
...
def agent(obs):
    ...
    # assign-2: secondary pass — any planet with surplus > MIN_CONTRIB also sends to top target
    MIN_CONTRIB = 10
    # find the highest-ROI target globally (the one most planets would want to send to)
    if targets and my_planets:
        all_cands = []
        for t in targets:
            speed_v = fleet_speed(t.ships + 1)
            for src in my_planets:
                if t.id in comet_planet_ids:
                    continue
                tx, ty = _converged_orbit_lead(t, src, initial_planets_map, angular_velocity, speed_v)
                if _path_safe(src.x, src.y, tx, ty, all_planets=planets, target_id=t.id, source_id=src.id):
                    all_cands.append((_roi(t, tx, ty, src), t, tx, ty))
        if all_cands:
            _, top_target, top_bx, top_by = max(all_cands, key=lambda x: x[0])
            top_angle = math.atan2(top_by - 0, top_bx - 0)  # direction computed per sender
            already_dispatched = {m[0] for m in moves}
            for mine in my_planets:
                if mine.id in departing_this_turn:
                    continue
                floor = max(mine.production * GARRISON_FLOOR_FACTOR, threat.get(mine.id, 0))
                surplus = mine.ships - floor
                if surplus > MIN_CONTRIB and mine.id not in already_dispatched:
                    sec_angle = math.atan2(top_by - mine.y, top_bx - mine.x)
                    moves.append([mine.id, sec_angle, MIN_CONTRIB])
