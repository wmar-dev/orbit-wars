...
def _run_game(args):
    ...
    return seed, rank, agent_reward


def run_evaluation(agent_path, opponent_path, num_games, jobs):
    ranks = []
    wins = 0
    game_args = [(agent_path, opponent_path, s) for s in range(num_games)]
    ...
    for seed, rank, reward in results:
        pending[seed] = (rank, reward)
        while next_to_print in pending:
            rank, reward = pending.pop(next_to_print)
...
def main():
    ...
    run_evaluation(args.agent, args.opponent, args.games, args.jobs)
