"""
Orbit Wars - Candidate I: Reactive Defense Dispatch (agent_v21)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Reactive defense (Candidate I): Before the offensive loop, scan obs.fleets
    for enemy fleets heading toward owned planets. If a planet will certainly be
    lost (projected_garrison at arrival < fleet.ships), dispatch targeted
    reinforcement from the nearest owned source with sufficient surplus. That
    source skips offensive dispatch this turn.

    Fleet tuple format: [id, owner, x, y, angle, from_planet_id, ships]
    Target inferred by dot-product alignment (>0.95) between fleet direction
    and fleet->owned-planet vector.

Hypothesis: agent_v20 loses planets that a targeted reinforcement could save.
Unlike Candidate C (10% vs v10, defended broadly), this only fires on
certain-loss scenarios with a source that can actually make a difference.
"""
...
def _detect_defense_needs(my_planets, fleets_raw, player):
    """Return dict: planet_id -> deficit for planets that will certainly be lost."""
    threats = {}
    for f in fleets_raw:
        if isinstance(f, (list, tuple)):
            ...
            f_x, f_y = getattr(f, "x", 0), getattr(f, "y", 0)
            f_angle = getattr(f, "angle", 0)
            f_ships = getattr(f, "ships", 0)

        if f_owner == player:
            continue

        for p in my_planets:
            dx, dy = p.x - f_x, p.y - f_y
            dist = math.hypot(dx, dy)
            if dist < 1.0:
                continue
            align = (math.cos(f_angle) * dx + math.sin(f_angle) * dy) / dist
            if align < 0.95:
                continue

            arrival_turns = dist / fleet_speed(f_ships)
            projected = p.ships + p.production * arrival_turns
            if projected >= f_ships:
                continue

            deficit = int(f_ships - projected) + 1
            if p.id not in threats or threats[p.id] < deficit:
                threats[p.id] = deficit
...
def agent(obs):
    ...
    fleets_raw = obs.get("fleets", []) if isinstance(obs, dict) else getattr(obs, "fleets", [])
    ...
    # Candidate I: Reactive defense — dispatch before offensive loop.
    threats = _detect_defense_needs(my_planets, fleets_raw, player)
    reinforcing_sources = set()
    planet_map = {p.id: p for p in planets}

    for threatened_pid, deficit in threats.items():
        t_planet = planet_map.get(threatened_pid)
        if t_planet is None:
            continue
        best_src = None
        best_dist = float('inf')
        for src in my_planets:
            if src.id in reinforcing_sources or src.id == threatened_pid or src.id in departing_this_turn:
                continue
            floor = _garrison_floor(src)
            surplus = src.ships - floor
            if surplus < deficit:
                continue
            dist = math.hypot(src.x - t_planet.x, src.y - t_planet.y)
            if dist < best_dist:
                best_dist = dist
                best_src = src
        if best_src is not None:
            angle = math.atan2(t_planet.y - best_src.y, t_planet.x - best_src.x)
            moves.append([best_src.id, angle, deficit])
            reinforcing_sources.add(best_src.id)

    # Single-sender coordination: pre-compute best sender per target.
    best_sender = {}
    for t in targets:
        best_score = float('inf')
        best_pid = None
        for src in my_planets:
            if src.id in departing_this_turn or src.id in reinforcing_sources:
                continue
            floor = _garrison_floor(src)
            surplus = src.ships - floor
            if surplus <= 0:
    ...
    for mine in my_planets:
        if mine.id in departing_this_turn or mine.id in reinforcing_sources:
            continue
