"""
Orbit Wars - Candidate F: Transit-Adjusted Fleet Sizing (agent_v17)

Builds on agent_v15 (single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Transit-adjusted fleet sizing (Candidate F): Estimates travel time as
    distance / fleet_speed(target.ships + 1) and sends
    target.ships + target.production * travel_turns + 1 ships to account for
    garrison growth during transit. Skips targets the source cannot afford.
    Prevents silent capture failures where the fleet arrives to find the target
    has grown past the launch-time ship count.
