"""
Orbit Wars - agent_v63 (experiments round 3)

Built on agent_v62 (best4 config, 70% vs v61, 72% vs v60).

Experiments round 3:
  PASS  DEFENSE_INTERCEPT_ENABLED    — inherited from v62, evaluated in round 3
  PASS  WEIGHTED_EVAL_FIXED_ENABLED   — corrected production-weighted beam eval
  PASS  SEARCH_DEPTH/BEAM_K            — deeper/wider beam search

Inherited from v62:
  PASS  SPLINTER_DISPATCH_ENABLED    — send surplus to nearest cheap neutral (62.5% vs v60)
  PASS  EVAL_ENHANCED_ENABLED        — planet count + ship count in beam eval (65% vs v60)
  PASS  OPPONENT_MODEL_V2_ENABLED    — proper position-based opponent in forward sim (80% vs v60)
  PASS  DYNAMIC_GARRISON_ENABLED     — lower garrison cap 2.5x/400t (67.5% vs v60)
"""
...
WEIGHTED_EVAL_FIXED_ENABLED    = True   # ON for sanity check (T010)
DEFENSE_INTERCEPT_ENABLED      = False  # DISCARDED — no benefit detected (48%/45% vs v62)
...
class _SimState:
    ...
    def step(self, opponent_model=False, player=-1):
        ...
        if OPPONENT_MODEL_V2_ENABLED and player >= 0:
...
def _greedy_moves(obs, planets, my_planets, targets, initial_planets_map,
    ...
    for mine in my_planets:
        ...
        if SPLINTER_DISPATCH_ENABLED and step <= SPLINTER_WINDOW:
