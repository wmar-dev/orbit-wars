"""
Orbit Wars - Candidate J: Smooth Adaptive Range (agent_v22)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Smooth adaptive range (Candidate J): Replaces the fixed RANGE_FACTOR = 2.0
    with a per-turn power-law formula:
      range_factor = clamp(2.0 * (own_total / max(1, enemy_total)) ** 0.25, 1.5, 3.5)
    where enemy_total = ships on opponent-owned planets (not neutral).
    At ratio=2.0 (winning), range_factor ≈ 2.38 (mild expansion).
    At ratio=0.5 (losing), range_factor ≈ 1.68 (gentle contraction).

Hypothesis: Candidate G used hard step-function thresholds (ratio ≤ 0.7 → 1.5)
that caused catastrophic contraction when losing; it scored 47.5% (19 draws, 1
loss) — neutral but not beneficial. The power-law avoids extreme contraction
while preserving winning-state expansion.
"""
...
def agent(obs):
    ...
    # Candidate J: Smooth adaptive range factor.
