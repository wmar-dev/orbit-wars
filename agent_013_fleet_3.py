"""
Orbit Wars - Experiment 013 fleet-3: race-aware fleet sizing
Base: agent_v38. Change: if enemy fleet heading to same target, scale up ships to win race.
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # fleet-3: race-aware override — if enemy fleet heading to same target, scale up
        RACE_EPSILON = 0.2
        for f in raw_fleets:
            if isinstance(f, (list, tuple)):
                f_owner, f_x, f_y, f_angle, f_ships = f[1], float(f[2]), float(f[3]), float(f[4]), int(f[6])
            ...
            if _angle_diff(f_angle, expected_angle) < RACE_EPSILON:
                # enemy fleet is racing to same target — send enough to beat it
                ships_needed = max(ships_needed, f_ships + best_target.ships + 1)
                break
