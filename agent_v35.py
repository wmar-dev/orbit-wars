"""
Orbit Wars - Candidate T Round 6: Transit-Adjusted Fleet Sizing (agent_v35)

Builds on agent_v33.

New mechanic — Candidate T (Round 6): Transit-adjusted fleet sizing
  Replaces `ships_needed = target.ships + 1` with a projected-garrison calculation:
    travel_turns = ceil(dist / fleet_speed(target.ships + 1))   [one fixed-point iter]
    projected_garrison = target.ships + target.production * travel_turns
    ships_needed = projected_garrison + 1
  Addresses the case where enemy planets with positive production accumulate ships
  during fleet transit, making the arriving fleet insufficient to capture the target.
  Most impactful for medium-distance enemy targets (travel time 10-30 turns).
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Candidate T (Round 6): transit-adjusted fleet sizing
        dist = math.hypot(mine.x - bx, mine.y - by)
