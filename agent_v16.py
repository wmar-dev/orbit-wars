"""
Orbit Wars - Candidate E: Speed-Corrected Orbit Lead (agent_v16)

Builds on agent_v15 (single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Speed-corrected orbit lead (Candidate E): Computes fleet_speed(target.ships + 1)
    per target inside the candidates loop, instead of the single fleet_speed(mine.ships + 1)
    computed once per source planet. Fixes a bug where the orbit-lead travel-time estimate
    used the source planet's full ship count rather than the actual launched fleet size,
    causing the predicted intercept position to be too early in the planet's orbit.
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for t in targets:
            ...
            # Use fleet_speed for the actual launched fleet size, not the source planet total.
