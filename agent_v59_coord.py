"""
Orbit Wars - agent_v59_coord

Experiment A: Fleet coordination via in-transit coverage check.
Base: agent_v58 (affordability fallback, 58% vs v57)

Change: Before dispatching to a target, check if an in-transit own fleet already
has sufficient ships en route to cover it (coverage[target_id] >= ships_needed).
If covered, skip — freeing the mine to target something else. After dispatching,
record the new coverage to prevent a second mine from doubling up.

Fixes: two mines independently targeting the same neutral while a better uncovered
target exists nearby.
"""
...
def agent(obs):
    ...
    # Build coverage dict: own in-transit fleet ships already heading to each target.
    # Used to skip targets that are already sufficiently covered, preventing two mines
    # from redundantly dispatching to the same planet.
    coverage = {}
    for f in raw_fleets:
        if isinstance(f, (list, tuple)):
            f_owner, f_x, f_y, f_angle, f_ships = f[1], float(f[2]), float(f[3]), float(f[4]), int(f[6])
        ...
        if f_owner != player:
            continue
        # Identify the planet this own fleet is heading toward
        best_match_id = None
        best_diff = math.pi
        for p in planets:
            expected = math.atan2(p.y - f_y, p.x - f_x)
            diff = _angle_diff(f_angle, expected)
            if diff < best_diff:
                best_diff = diff
                best_match_id = p.id
    ...
    for mine in my_planets:
        ...
        for item in sorted_candidates:
            ...
            # Coverage check: skip if an in-transit own fleet already covers this target
