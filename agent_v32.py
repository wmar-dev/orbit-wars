"""
Orbit Wars - Bug-Fixed Baseline Agent (agent_v32)

Builds on agent_v31 (reward-blend scoring) with two critical bug fixes.

Eval result: 64% score vs agent_v31 (32W/18L/0D over 50 games, seeds 0-49).
Mean per-turn reward delta: +0.0288 (v32=0.1258 vs v31=0.0970).

Bug fixes applied:
  - Fix 1 — Comet remaining-turns (Bug: remaining_steps field was undocumented and
    always 0, so evacuate_next_turn never fired; ships destroyed on comet exit).
    Fix: compute remaining_turns = max(0, len(path) - path_index) from documented
    paths/path_index fields (CONTEST.md). Evacuation triggers at remaining_turns <=
    EVACUATE_THRESHOLD=3. Target pool now includes owned planets (reinforce) +
    non-owned (capture), all scored by unified ROI.

  - Fix 2 — Converged orbit-lead (Bug: 2-iteration _refined_orbit_lead diverged for
    fast-orbiting planets, up to ~1.2 units error at angular_velocity=0.05 rad/turn).
    Fix: _converged_orbit_lead fixed-point loop (delta < ORBIT_LEAD_EPS=0.1 or
    ORBIT_LEAD_MAX_ITER=10 cap, typically 4-6 iterations). Comet targeting uses
    _comet_two_pass (two-pass re-estimate) instead of single-step lookup.

Candidates NOT included in experiments round (all retested vs v32):
  - Candidate P (3-iter orbit lead): superseded — fully incorporated into Fix 2
  - Candidate J (smooth adaptive range): inapplicable — range cap removed in v30 (Q)
  - Candidate K (enemy-territory priority): 50% (20 draws) -- FAIL (symmetric self-play)
  - Candidate R (production-squared ROI): 45% -- FAIL
  - Candidate L (two-source attack): 40% -- FAIL
  - Candidate I (reactive defense): 5% -- FAIL

Inherited unchanged from agent_v31:
  - Candidate S: Reward-blend scoring (REWARD_ALPHA=0.1)
  - Candidate D: Single-sender coordination
  - Candidate H: Capture ROI scoring
...
# Comet evacuation threshold (Fix 1)
EVACUATE_THRESHOLD = 3  # evacuate when remaining_turns <= this value

# Converged orbit-lead parameters (Fix 2)
ORBIT_LEAD_EPS = 0.1    # convergence tolerance in units
ORBIT_LEAD_MAX_ITER = 10  # iteration cap to prevent oscillation

# Reward-blend config (Candidate S, inherited from v31)
REWARD_ALPHA = 0.1
...
def _converged_orbit_lead(t, mine, initial_planets_map, angular_velocity, speed,
    ...
    """Fixed-point orbit-lead: iterate until intercept estimate converges or cap reached.

    Replaces _refined_orbit_lead (2 fixed iterations). Converges to <eps units
    error in 4-6 iterations for typical angular_velocity values (0.025-0.05 rad/turn).
    """
    x, y = t.x, t.y
...
def _build_comet_path_lookup(obs):
    """Build per-comet (path, path_index, remaining_turns) lookup.

    remaining_turns computed from len(path) - path_index per CONTEST.md documented
    fields. The remaining_steps field is NOT used (undocumented, unreliable).
    """
    lookup = {}
...
def _comet_two_pass(comet_planet, mine_x, mine_y, comet_path_lookup, speed):
    """Two-pass comet intercept: re-estimate travel time after first position prediction.

    Corrects for the fact that the comet moves while the fleet is in transit.
    Returns (x_pred, y_pred, valid).
    """
    # Pass 1: estimate from current comet position
    ...
    if not valid1:
        return comet_planet.x, comet_planet.y, False
    # Pass 2: re-estimate from predicted position
...
def _reward_estimate(target, dispatch_ships):
    """Candidate S: Forward-looking reward estimate for capturing `target`."""
...
def agent(obs):
    ...
    # Fix 1: use computed remaining_turns (not undocumented remaining_steps field)
    ...
    # Single-sender coordination (Candidate D)
    best_sender = {}
    for t in targets:
    ...
    for mine in my_planets:
        ...
        # Fix 1: Evacuate comet — dispatch all ships to best safe destination.
        # Target pool: all planets except mine itself (owned=reinforce, non-owned=capture).
        if mine.id in evacuate_this_turn:
            ...
            for p in planets:
                ...
                # Score: owned planets by reinforce value, non-owned by ROI
                if p.owner == player:
        ...
        # Standard offense: only targets where this planet is the designated best sender.
        candidates = []
        for t in targets:
        ...
        # Candidate S: Blend ROI with reward estimate.
