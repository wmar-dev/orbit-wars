"""
Orbit Wars - agent_v58_efficiency

Experiment B: Growth-efficiency scoring + affordability fallback.
Base: agent_v58_fallback (affordability fallback with quality guard, 58% vs v57)

Change: For neutral planet candidates, replace the ROI-based blended_key score
with production / max(ships, 1) (growth-efficiency). This directly measures how
many ships of growth are gained per ship invested in capture, without the squared
production weighting and without the travel-time penalty in ROI that inflated
Planet 16's apparent advantage over Planet 8.

Enemy planet scoring is unchanged (still uses _roi).
Affordability fallback and FALLBACK_MIN_RATIO quality guard are inherited unchanged.
"""
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Precompute growth-efficiency for neutral candidates for use in blended_key.
        # growth_efficiency = production / ships: ships of future growth per ship invested.
        max_efficiency = max(
            (t.production / max(t.ships, 1) for _, t, _, _ in roi_scores if t.owner == -1),
            default=1.0
        ) or 1.0

        def blended_key(item, _max_roi=max_roi, _max_eff=max_efficiency):
            roi, t, bx, by = item
            if t.owner == -1:
                # Growth-efficiency score for neutrals: normalize by max efficiency in candidates
                score = t.production / max(t.ships, 1) / _max_eff
            else:
