"""
Orbit Wars - Candidate R: Production-Squared ROI Bias (agent_v29)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Production-squared ROI (Candidate R): Squares the production term in the
    ROI formula: production^2 * (100-travel) / cost. Creates much stronger
    preference for high-production targets, concentrating attacks on the most
    valuable planets unconditionally from turn 1.

Hypothesis: Linear production weighting doesn't differentiate strongly enough
between prod=3 and prod=5 planets. Squaring creates decisive early advantages
by always targeting the highest-production planets first.
"""
...
def _roi(t, bx, by, mine):
    """Candidate R: production^2 * remaining_turns / capture_cost."""
