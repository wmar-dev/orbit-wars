"""
Orbit Wars - Combined Agent Round 5 (agent_v33)

Builds on agent_v32 (bug-fixed baseline: converged orbit-lead + comet evacuation).

New mechanic — Candidate R: Production-Squared ROI (this version)
  Changes the ROI numerator from `t.production` to `t.production ** 2`, creating a
  stronger preference for high-production planets. Failed vs v20 (45%) because targeting
  bugs caused misses on high-production orbiting targets. With converged orbit-lead in
  v32, those targets are now reliably intercepted, making the amplified bias pay off.
  Tested: 60% vs v32 (PASS). All other candidates retested vs v32 (see experiments/).

Candidates NOT stacked (all retested vs v32):
  - Candidate P (3-iter orbit lead): superseded by converged orbit-lead in v32
  - Candidate J (smooth adaptive range): inapplicable (range cap removed in v30)
  - Candidate K (enemy-territory priority): 50% (50 draws) -- FAIL
  - Candidate L (two-source attack): 20% -- FAIL
  - Candidate I (reactive defense): 16% -- FAIL

Safety guards and base logic inherited from agent_v32 (unchanged):
  - Fix 1: comet remaining_turns from len(path) - path_index (CONTEST.md fields)
  - Fix 2: _converged_orbit_lead + _comet_two_pass (accurate intercept)
...
def _roi(t, bx, by, mine):
    """Candidate R: Production-squared ROI — amplifies preference for high-production targets."""
