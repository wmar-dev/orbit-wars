"""
Orbit Wars - agent_v59_defense

Experiment B: Defensive reinforcement pre-pass.
Base: agent_v58 (affordability fallback, 58% vs v57)

Change: Added _threat_eta() helper that finds the nearest enemy fleet heading to
an owned planet and estimates its arrival time. Added a defense pre-pass inside
agent() that runs after the threat dict is built: for each threatened planet with
production >= DEFENSE_MIN_PRODUCTION, if the planet cannot hold on its own, finds
the nearest allied planet with sufficient surplus that can arrive before the enemy,
and dispatches reinforcement. Dispatched defenders are skipped in the normal loop.
"""
...
def _threat_eta(planet, raw_fleets, player):
    ...
    Returns (0, 9999) if no threat detected."""
    worst_ships = 0
    worst_eta = 9999
    for f in raw_fleets:
...
def agent(obs):
    ...
    # Defense pre-pass: dispatch reinforcement to threatened high-value planets.
    # Defenders are excluded from the main dispatch loop via dispatched_defenders.
    ...
    for p in my_planets:
        ...
        if ships_at_arrival >= incoming:
            continue  # planet holds on its own
        # Ships needed accounting for planet growth during reinforcement travel
        ...
        for src in sorted(my_planets, key=lambda q: math.hypot(q.x - p.x, q.y - p.y)):
            ...
            # Don't sacrifice more than 50% of source ships for defense
            ...
            if src.ships - needed < floor_src:
                continue  # can't afford while maintaining garrison
    ...
    for mine in my_planets:
        ...
        if mine.id in dispatched_defenders:
            continue  # already dispatched a defensive fleet this turn
