...
| `agent_v48.py` | + ROI scoring mismatch fix: pre-compute ships_needed for enemy candidates; pass actual fleet size to _roi (Candidate C1 R015) | 54% win rate vs v47 (50 games) — FAIL |
| `agent_v49.py` | + Endgame ROI normalization: replace hardcoded 100.0 with remaining_turns=500-step (Candidate C2 R015) | 48% win rate vs v47 (50 games) — FAIL |
| `agent_v50.py` | + Garrison defense buffer: add production×2 above raw threat count so planet survives attack with non-zero garrison (Candidate C3 R015) | 62% win rate vs v47 (50 games); 76% win rate vs v38 (50 games) |
| `agent_v51.py` | + Sender pre-screening: exclude senders that can't cover enemy garrison estimate (Candidate C4 R015) | 16% win rate vs v47 (50 games) — FAIL (severe regression) |
...
| `agent_v58.py` | Candidate B: multi-planet dispatch — remove single-sender serialization; each planet finds its own best ROI target independently | Kaggle: 851.0–880.7; loses to agent_v64 52.0% h2h (50 games, round 6 baseline matrix) |
...
| `agent_v68.py` | Round 7: opening-game idle-planet rush candidate added to beam search (full-fleet rush to cheapest affordable target for step ≤ 20) — established-game variant (step > 20) discarded as severe regression | 64.0% (32/50, --swap) vs agent_v64; 0.0% (0/30, --swap) vs slawekbiel_agent (benchmark) — tied with agent_v64's 0.0% (0/30), no regression |
| **`agent_v69.py`** | **Round 8: global coordinated allocation — joint (source, target) assignment replaces per-planet greedy target claiming (Candidate A); deeper iterative-deepening search (Candidate B, 44.0% — FAIL, search-pathology regression) and regroup/reinforcement repositioning (Candidate C, 8.0% — FAIL, flawed stress heuristic drains active-front surplus into idle rear planets) both discarded** | **56.0% (28/50, --swap) vs agent_v68; 0.0% (0/30, --swap) vs slawekbiel_agent (benchmark) — tied with agent_v68's 0.0% (0/30), no regression — current best** |
...
**`agent_v57.py`**: Fixes fleet targeting when both source and target are orbiting. Fix A: the orbit lead previously used the planet center as the launch origin, but the game engine launches fleets from `planet.center + direction × (radius + 0.1)` — a new `_launch_corrected_orbit_lead` wrapper re-runs the orbit lead from the actual launch position after a first-pass direction estimate. Fix B: `_path_safe` now predicts intermediate orbiting planet positions at the midpoint of the fleet's travel time rather than using current positions, eliminating false-negative dispatch blocks when intermediate planets happen to be in the current line-of-sight but will have moved by arrival. Achieves 61% win rate vs agent_v56 across 200 games.

**`agent_v58.py`**: Fixes early-game dispatch delay found via replay analysis (replay 78539022). Root cause: the targeting loop selected the highest-ROI neutral unconditionally, then bailed if the mine lacked ships — it never tried cheaper but affordable alternatives. Example: Planet 16 (ROI 27.20, needs 31 ships) was unaffordable at step 6, but Planet 8 (ROI 20.38, needs 19 ships) was available and never reached; the agent waited 6 more steps. Fix: sort candidates by blended ROI descending, iterate until finding one that is (a) affordable and (b) scores ≥70% of the best candidate's blended score (quality guard prevents chasing low-value cheap planets). Achieves 58% win rate vs agent_v57 across 50 games.
