"""
Orbit Wars - Candidate O: Lower Garrison Floor (agent_v26)

Builds on agent_v20 (speed-corrected orbit lead + capture-ROI scoring +
single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Lower garrison floor (Candidate O): Reduces GARRISON_FLOOR_FACTOR from 5
    to 3, allowing each planet to dispatch more ships per turn. Enables faster
    captures and breaks symmetric-draw pattern from ratio-gated mechanics.

Hypothesis: GARRISON_FLOOR_FACTOR=5 is conservative; lowering to 3 creates
more aggressive dispatching unconditionally, producing decisive outcomes.
"""
