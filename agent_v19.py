"""
Orbit Wars - Candidate H: Capture-ROI Scoring (agent_v19)

Builds on agent_v15 (single-sender coordination + all agent_v10 safety guards).

Added mechanic:
  - Capture-ROI scoring (Candidate H): Replaces the production/distance target
    score with production * max(1, 100 - travel_turns) / max(1, target.ships +
    target.production * travel_turns + 1). This normalizes by capture cost and
    remaining turns owned, rewarding planets that are cheap to capture early and
    have time to produce. travel_turns = distance / fleet_speed(target.ships + 1).
...
def _roi(t, bx, by, mine):
    """Capture ROI: production * remaining_turns / capture_cost."""
