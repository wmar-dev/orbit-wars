"""
Orbit Wars - Candidate S Round 6: Cross-Turn Fleet Deduplication (agent_v34)

Builds on agent_v33.

New mechanic — Candidate S (Round 6): In-transit fleet deduplication
  Parses obs.fleets to detect friendly fleets already heading toward each target.
  Uses angle-matching (ANGLE_EPSILON = 0.1 rad) from each fleet's current position
  to each target's current position to infer destination without a to_planet_id field.
  Deducts in-transit ships from ships_needed: if a target is already funded by
  in-transit fleets, dispatch is skipped or reduced. Prevents cross-turn redundant
  sends that waste ships on already-covered targets (single-sender coordination from
  Candidate D prevents same-turn redundancy; this handles the cross-turn case).
...
REWARD_ALPHA = 0.1
ANGLE_EPSILON = 0.1  # radians: threshold for fleet-to-target angle matching
...
def agent(obs):
    ...
    # Candidate S (Round 6): build in_transit dict from obs.fleets
    in_transit = {}
    for f in raw_fleets:
        ...
        for t in targets:
            ...
            if _angle_diff(f_angle, expected) < ANGLE_EPSILON:
                in_transit[t.id] = in_transit.get(t.id, 0) + f_ships
    ...
    for mine in my_planets:
        ...
        # Candidate S (Round 6): deduct already in-transit friendly ships
        raw_needed = best_target.ships + 1 - in_transit.get(best_target.id, 0)
        if raw_needed <= 0:
            continue  # target already funded by in-transit fleet(s)
        ships_needed = raw_needed
