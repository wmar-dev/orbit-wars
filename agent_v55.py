"""
Orbit Wars - agent_v55

Candidate: C1 + C2 combined (ROI mismatch fix + endgame ROI normalization)
Base: agent_v48 (C1)

C1 (ROI mismatch fix): pre-process enemy candidates through _enemy_fleet_size
upfront so _roi uses actual_fleet_size=ships_needed. Enemy targets travel faster
(larger fleet) than fleet_speed(target.ships+1) estimates, so they were scored
as less attractive than they really are.

C2 (endgame ROI normalization): replace hardcoded 100.0 time-decay with
remaining_turns = max(1.0, 500.0 - step). Far targets are correctly penalized
in late-game when there is less production time remaining before the game ends.
"""
...
def _roi(t, bx, by, mine, actual_fleet_size=None, remaining_turns=100.0):
...
def agent(obs):
    ...
    game_remaining = max(1.0, 500.0 - step)
    ...
    for mine in my_planets:
        ...
        if mine.id in evacuate_this_turn:
            ...
            for p in planets:
                ...
                if p.owner == player:
                    ...
                    score = _roi(p, x_pred, y_pred, mine, remaining_turns=game_remaining)
        ...
        roi_scores = [(_roi(t, bx, by, mine, actual_fleet_size=sn, remaining_turns=game_remaining), t, bx, by, sn)
