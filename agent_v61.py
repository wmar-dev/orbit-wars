"""
Orbit Wars - agent_v61

Three targeted tactical improvements over agent_v60 (Kaggle 916.9), each
addressing a measured gap from replay analysis against top competitors:

  1. Early-dispatch (EARLY_DISPATCH_ENABLED): In turns 0-EARLY_DISPATCH_WINDOW,
     prioritize the nearest capturable neutral over the best-ROI target so we
     do not stockpile ships while the opponent secures an early production lead.

  2. Dynamic garrison floor (DYNAMIC_GARRISON_ENABLED): Cap garrison multiplier
     at 2.5x (ramp over 400 turns) instead of 4x (ramp over 300 turns), freeing
     surplus ships for dispatch in the mid-game.

  3. Production-weighted eval (WEIGHTED_EVAL_ENABLED): Beam search accumulates
     score each simulated turn rather than sampling only at the horizon, so
     fast captures score higher than slow ones of equal final value.

Each direction is independently togglable for A/B evaluation.
Base: agent_v60 (beam lookahead, depth=10, BEAM_K=3)
"""
...
# ---------------------------------------------------------------------------
# Tactical improvement toggles (v61) — set False to isolate/disable each
# ---------------------------------------------------------------------------

EARLY_DISPATCH_ENABLED    = False  # Direction 1: DISABLED — pre-pass disrupts ROI coordination (36% vs v60)
EARLY_DISPATCH_WINDOW     = 15     # last turn where early-dispatch applies
DYNAMIC_GARRISON_ENABLED  = True   # Direction 2: lower gff cap (2.5x) + slower ramp (56% vs v60)
WEIGHTED_EVAL_ENABLED     = False  # Direction 3: DISABLED — transit accumulation inflates dispatch (40% vs v60)
...
def _greedy_moves(obs, planets, my_planets, targets, initial_planets_map,
    ...
    if EARLY_DISPATCH_ENABLED and step <= EARLY_DISPATCH_WINDOW:
        for mine in my_planets:
            ...
            best_early_pos = (0.0, 0.0)
            best_early_needed = 0
            for t in targets:
                if t.owner != -1:
                    continue
                if t.id in early_claimed or t.id in comet_planet_ids:
                    continue
                speed = fleet_speed(t.ships + 1)
                x_pred, y_pred = _converged_orbit_lead(
                    t, mine, initial_planets_map, angular_velocity, speed)
                ...
                needed = int(t.ships + t.production * travel_turns) + 1
                if mine.ships < needed:
                    continue
                # Score: production per turn captured — prefers fast high-value captures
                score = t.production / (travel_turns + 1.0)
                if score > best_early_score:
                    best_early_score = score
                    best_early = t
                    best_early_pos = (x_pred, y_pred)
                    best_early_needed = needed
            if best_early is not None:
                early_claimed.add(best_early.id)
                early_dispatched.add(mine.id)
                angle = math.atan2(best_early_pos[1] - mine.y, best_early_pos[0] - mine.x)
                moves.append([mine.id, angle, best_early_needed])

    claimed_targets = set(early_claimed)

    for mine in my_planets:
        if mine.id in departing_this_turn:
            continue
        if mine.id in early_dispatched:
            continue
