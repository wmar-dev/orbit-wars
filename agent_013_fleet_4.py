"""
Orbit Wars - Experiment 013 fleet-4: combined production-buffered + race-aware sizing
Base: agent_v38. Change: buffer for production accrual AND race-awareness combined.
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # fleet-4: production-buffered + race-aware
        dist_to_target = math.hypot(bx - mine.x, by - mine.y)
        travel_turns = dist_to_target / fleet_speed(best_target.ships + 1)
        ships_needed = best_target.ships + 1 + int(math.ceil(best_target.production * travel_turns))
        RACE_EPSILON = 0.2
        for f in raw_fleets:
            if isinstance(f, (list, tuple)):
                f_owner, f_x, f_y, f_angle, f_ships = f[1], float(f[2]), float(f[3]), float(f[4]), int(f[6])
            ...
            if f_owner == player:
                continue
            expected_angle = math.atan2(by - f_y, bx - f_x)
            if _angle_diff(f_angle, expected_angle) < RACE_EPSILON:
                enemy_travel = dist_to_target / fleet_speed(f_ships)
                target_ships_at_arrival = best_target.ships + int(best_target.production * enemy_travel)
                ships_needed = max(ships_needed, f_ships + target_ships_at_arrival + 1)
                break
