...
def agent(obs):
    ...
    for t in targets:
        ...
        for src in my_planets:
            ...
            surplus = src.ships - floor - committed.get(src.id, 0)
            if surplus <= 0:
                continue
    ...
    for mine in my_planets:
        ...
        if mine.id in evacuate_this_turn:
            avail_evac = mine.ships - committed.get(mine.id, 0)
            if avail_evac < 1:
                continue
            ...
            moves.append([mine.id, angle, avail_evac])
            committed[mine.id] = committed.get(mine.id, 0) + avail_evac
            continue
        ...
        if mine.ships - committed.get(mine.id, 0) < ships_needed:
            continue
