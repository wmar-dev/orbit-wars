"""
Orbit Wars - Candidate B: Garrison Sizing (agent_v12)

Builds on agent_v10 (intermediate planet obstruction + orbit-lead refinement).

Mechanic added:
  Garrison sizing — each source planet retains a garrison floor before launching,
  and sends exactly min(target.ships + 1, available_surplus) ships instead of
  a fixed fraction. Three sub-experiments are implemented via GARRISON_FLOOR_MODE:
    Mode A: garrison_floor = max(production * 5, 1)
    Mode B: garrison_floor = max(production * 10, 1)
    Mode C: garrison_floor = 10 (fixed)
  The mode with the highest win rate vs agent_v10 over 20 games is selected as
  the permanent value. Current active mode: set GARRISON_FLOOR_MODE below.

Evaluation result: All three modes scored 0% win rate vs agent_v10 (20 games each). No winning variant.
  Mode A (production×5): 0/20. Mode B (production×10): 0/20. Mode C (fixed 10): 0/20.
  Garrison floor starves early-game expansion; agent remains on Mode A as the least restrictive.
"""
...
# Sub-experiment selector: 'A', 'B', or 'C'
GARRISON_FLOOR_MODE = 'A'
...
def _garrison_floor(planet):
    if GARRISON_FLOOR_MODE == 'A':
        return max(planet.production * 5, 1)
    elif GARRISON_FLOOR_MODE == 'B':
        return max(planet.production * 10, 1)
    else:  # Mode C
        return 10
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Candidate B: skip if no surplus after garrison floor.
        if available <= 0:
            continue
        ...
        # Candidate B: send exactly enough to capture, capped by available surplus.
        send = min(best_target.ships + 1, available)
        if send <= 0:
            continue
