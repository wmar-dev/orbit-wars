"""
Orbit Wars - Combined Agent (agent_v15)

Builds on agent_v10 (intermediate planet obstruction + orbit-lead refinement).

Stacked mechanics (only mechanics that passed ≥55% vs agent_v10 are included):
  - Single-sender coordination (Candidate D, agent_v14): For each target, only
    the planet with the minimum efficiency score (distance / available_surplus)
    may launch. Other planets redirect to different targets, spreading attack
    vectors across the map.

Candidates NOT included (failed ≥55% threshold):
  - Candidate A (redundant fleet avoidance): 10% win rate vs v10 — FAIL
  - Candidate B (garrison sizing): 0% win rate vs v10 — FAIL
  - Candidate C (threat-aware defense): 10% win rate vs v10 — FAIL
...
Evaluation result: 70% win rate vs agent_v10 (20 games, seeds 0–19); 0 sun losses; 0 OOB losses
"""
