"""
Orbit Wars - agent_v42

Builds on agent_v38 (experiment 013).

Experiment 013 result: dynamic garrison floor is the only improvement found.
Scoring formula, fleet sizing, and source assignment all validated as already optimal.

New mechanic — Dynamic Garrison Floor:
  Instead of a static GARRISON_FLOOR_FACTOR=3, the floor factor ramps from 1.0 at step 0
  to 4.0 at step 300+ (linear interpolation). This enables aggressive early expansion
  (low floor → more surplus → more planet captures) while protecting garrisons in late
  game (high floor → harder for enemy to capture undefended planets).
  Formula: factor = 1.0 + 3.0 * min(step / 300.0, 1.0)

Result: 54% score vs agent_v38 (27W/23L/0D, 50 games, seeds 0-49).

All other mechanics inherited unchanged from agent_v38:
  - ROI = prod² × decay / cost (validated as optimal over 3 alternatives)
  - Fleet sizing: target.ships + 1 (validated as optimal over 3 alternatives)
  - Single-sender coordination (validated as optimal over 2 multi-sender alternatives)
  - Candidate U: threat-aware garrison floor (floor raised to incoming threat if higher)
  - Fix 2: Converged orbit-lead + two-pass comet intercept
  - Fix 1: Comet evacuation
  - Candidate D: Single-sender
  - Candidate O: GARRISON_FLOOR_FACTOR baseline (now replaced by dynamic)
  - Candidate Q: No range cap
  - _path_safe(): full-ray sun check + planet obstruction
"""
...
def agent(obs):
    ...
    # floor-5: dynamic garrison factor ramps from 1.0 at step 0 to 4.0 at step 300+
