...
Experiments round 5:
  DISCARDED  MULTI_SOURCE_ENABLED       — multi-source coordinated attacks (12% vs v64)
  DISCARDED  FLEET_SIZE_OPT_ENABLED     — iterative fleet size convergence (10% vs v64)
  DISCARDED  FFA_ADAPT_ENABLED          — 4-player state adaptation (48% vs v64)
  DISCARDED  ENDGAME_FOCUS_ENABLED      — endgame elimination focus (0% vs v64)
...
MULTI_SOURCE_ENABLED      = False  # P1 — DISCARDED (12% vs v64)
FLEET_SIZE_OPT_ENABLED    = False  # P2 — DISCARDED (10% vs v64)
FFA_ADAPT_ENABLED         = False  # P3 — DISCARDED (48% vs v64)
ENDGAME_FOCUS_ENABLED     = False  # P4 — DISCARDED (0% vs v64)
...
def fleet_speed(n):
    if n <= 0:
        return 1.0
    if n in _FLEET_SPEED_CACHE:
        return _FLEET_SPEED_CACHE[n]
    v = 1.0 + 5.0 * (math.log(n) / math.log(1000)) ** 1.5
    _FLEET_SPEED_CACHE[n] = v
    return v
...
def _greedy_moves(obs, planets, my_planets, targets, initial_planets_map,
    ...
    if FFA_ADAPT_ENABLED:
        ...
        ffa_mult = 1.0
        splinter_window = SPLINTER_WINDOW
    if DYNAMIC_GARRISON_ENABLED:
        gff = ffa_mult * (1.0 + 1.5 * min(step / 400.0, 1.0) * phase_mult)
    else:
        gff = ffa_mult * (1.0 + 3.0 * min(step / 300.0, 1.0) * phase_mult)

    if ENDGAME_FOCUS_ENABLED:
        opp_count = _count_opponents(planets, player)
        if opp_count <= 1:
            targets = [t for t in targets if t.owner >= 0]
            gff *= 0.85
...
def _gen_beam_candidates(my_planets, targets, greedy_moves, planets, initial_planets_map,
    ...
    # P1: Multi-source coordinated attack candidates
    if MULTI_SOURCE_ENABLED:
        ...
        for target_id, sources in target_to_sources.items():
            if len(sources) < 2:
                continue
            mine_a, ships_a, _, _ = sources[0]
            mine_b, ships_b, _, _ = sources[1]
            _, _, x_a, y_a = _compute_top_k_targets(
                mine_a, targets, initial_planets_map, angular_velocity,
                comet_path_lookup, comet_planet_ids, planets, k=1
            )[0] if _compute_top_k_targets(
                mine_a, targets, initial_planets_map, angular_velocity,
                comet_path_lookup, comet_planet_ids, planets, k=1
            ) else (None, None, 0.0, 0.0)
            _, _, x_b, y_b = _compute_top_k_targets(
                mine_b, targets, initial_planets_map, angular_velocity,
                comet_path_lookup, comet_planet_ids, planets, k=1
            )[0] if _compute_top_k_targets(
                mine_b, targets, initial_planets_map, angular_velocity,
                comet_path_lookup, comet_planet_ids, planets, k=1
            ...
            if tgt is None:
                continue
            dist_a = math.hypot(tgt.x - mine_a.x, tgt.y - mine_a.y)
            dist_b = math.hypot(tgt.x - mine_b.x, tgt.y - mine_b.y)
            eta_a = max(1, int(dist_a / fleet_speed(ships_a)))
            eta_b = max(1, int(dist_b / fleet_speed(ships_b)))
            d_a = (mine_a.id, target_id, ships_a, eta_a)
            d_b = (mine_b.id, target_id, ships_b, eta_b)
            angle_a = math.atan2(tgt.y - mine_a.y, tgt.x - mine_a.x)
            angle_b = math.atan2(tgt.y - mine_b.y, tgt.x - mine_b.x)
            alt_dispatches = [d for d in greedy_dispatches if d[0] not in (mine_a.id, mine_b.id)] + [d_a, d_b]
            alt_moves = [m for m in greedy_moves if m[0] not in (mine_a.id, mine_b.id)] + [[mine_a.id, angle_a, ships_a], [mine_b.id, angle_b, ships_b]]
            candidates.append((alt_dispatches, alt_moves))
