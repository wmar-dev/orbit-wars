...
Candidate A: Early-game aggressive dispatch for neutral planets.
Base: agent_v56

Replay analysis showed we stockpile 50+ ships on home planet in turns 0-10
while the opponent captures 2 neutrals. Root cause: we only dispatch when
mine.ships >= ships_needed (full capture). In early game (step < 60), relax
this: if the target is neutral and we have positive surplus above garrison floor,
send all our surplus immediately — don't wait to accumulate the full amount.

This races us to neutrals in turns 0-15 instead of turns 8-20.
"""
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # Candidate A: in early game, send surplus immediately toward neutrals
        # rather than waiting to accumulate the full ships_needed amount.
        if mine.ships < ships_needed:
            if step < 60 and best_target.owner == -1:
                incoming = threat.get(mine.id, 0)
                buffer = mine.production * 2 if incoming > 0 else 0
                floor = max(mine.production * GARRISON_FLOOR_FACTOR, incoming + buffer)
                surplus = int(mine.ships - floor)
                if surplus >= 1:
                    ships_needed = surplus
                else:
                    continue
            else:
                continue
